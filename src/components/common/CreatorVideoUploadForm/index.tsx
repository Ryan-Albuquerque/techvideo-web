...
export default function CreatorVideoUploadForm() {
  ...
  const handleUploadVideo = async (event: FormEvent<HTMLFormElement>) => {
    try {
      ...
      const audioFile = await ffmpegResource.convertVideoToAudio(frontFile);
      ...
      const transcriptionResponse = await startTask(
        `/video/${videoId}/transcription`,
        { prompt: transcriptionPromptTextArea }
      );
      ...
      Notification(
        ...
        "Video uploaded, now you can select your prompt generation type!"
      );
    } catch (error: any) {
      setUploadStatus(StatusToButtonEnum.READ);
      Notification(
    ...
    <>
      <form className="w-full space-y-6" onSubmit={handleUploadVideo}>
        ...
        <div className="space-y-2">
          <Label className="max-md:text-xs" htmlFor="transcription_prompt">
