...
export default function GenerateByVideo() {
    ...
    const [uploadStatus, setUploadStatus] = useState<StatusToButtonEnum>(StatusToButtonEnum.DISABLED)
    const [generatorType, setGeneratorType] = useState<string>('video_transcription')
    ...
    const handleUploadVideo = async (event: FormEvent<HTMLFormElement>) => {
        try {            
            ...
            setId(videoId)
            const {data: transcriptionData} = await api.post(`/video/${videoId}/transcription`, {})
            ...
            <main className="flex-1 flex min-h-screen max-sm:flex-col">
                <div className="flex flex-col flex-1 gap-4 max-sm:flex-none space-y-8 p-5">
                    <form className="flex flex-col gap-4 p-5 flex-1 max-md:p-1 max-md:pt-5" onSubmit={handleUploadVideo}>
                        ...
                        <Select defaultValue="video_transcription" onValueChange={(e: string) => setGeneratorType(e)}>
                            <SelectTrigger>
                                <SelectValue placeholder="Select a prompt" />
                            </SelectTrigger>
                            <SelectContent >
                                <SelectGroup>
                                    <SelectItem value="video_transcription">Video Transcription</SelectItem>
                                    <SelectItem value="video_summary">Video Summary</SelectItem>
                                </SelectGroup>
                            </SelectContent>
                        </Select>

                        <Button type='submit' className='w-full my-3' disabled={uploadStatus == StatusToButtonEnum.DISABLED}>
